...
class AssessmentTemplate(TenantBase):
    ...
    framework_ids: Mapped[list[uuid.UUID] | None] = mapped_column(
        ARRAY(UUID(as_uuid=True)), nullable=True
    )
    tier_applicability: Mapped[list[str] | None] = mapped_column(
...
class Question(TenantBase):
    ...
    risk_domain: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class Assessment(TenantBase):
    ...
    reminder_schedule: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )
...
class QuestionnaireResponse(TenantBase):
    ...
    response_options: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ai_prefilled: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )
    ai_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)
    ai_citations: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
