"""
FastAPI application for Velora Communication Service.
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    yield
    await close_engine()
    logger.info("service_stopped", service="communication")


def create_app() -> FastAPI:
    """Construct the FastAPI application."""
    app = FastAPI(
        title="Velora Communication Service",
        version="2.0.0",
        lifespan=lifespan,
        docs_url="/docs",
        redoc_url="/redoc",
    ...
    async def health() -> dict:
        return {"status": "healthy", "service": "communication"}
